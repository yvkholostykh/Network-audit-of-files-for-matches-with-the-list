...
# --- 1. ПОДКЛЮЧЕНИЕ ЛОКАЛЬНЫХ БИБЛИОТЕК (с авто‑поиском подпапок) ---
BASE_DIR = Path(r"C:/Users/kholostykh.iuv/Desktop/FileAuditor")

# Базовые пути к папкам библиотек
LIB_PATHS = {
...
def add_all_subdirs_to_syspath(base_path):
    """Добавляет в sys.path саму папку и все её подпапки, где есть __init__.py"""
    if not base_path.exists():
        return
    # Добавляем корень
    sys.path.insert(0, str(base_path))
    # Ищем все подкаталоги, содержащие __init__.py (настоящие пакеты)
    for dirpath, dirnames, filenames in os.walk(base_path):
        if '__init__.py' in filenames:
            sys.path.insert(0, dirpath)

# Обрабатываем все библиотеки
for path in LIB_PATHS.values():
    add_all_subdirs_to_syspath(path)
...
# alive_progress и его зависимость about_time
try:
    import about_time
except ImportError:
    # Пробуем найти about_time внутри alive-progress-main
    abt_path = BASE_DIR / "alive-progress-main" / "about_time"
    if abt_path.exists():
        sys.path.insert(0, str(abt_path))
    else:
        # maybe it's a single .py
        abt_py = BASE_DIR / "alive-progress-main" / "about_time.py"
        if abt_py.exists():
            sys.path.insert(0, str(abt_py.parent))
    try:
        import about_time
    except ImportError:
        pass
...
class FileAuditorPro:
    ...
    def save_report_json(self, output_path):
        try:
            with open(output_path, 'w', encoding='utf-8') as jf:
                json.dump(self.results, jf, ensure_ascii=False, indent=2)
            print(f"{Fore.GREEN}📁 JSON сохранён: {output_path}")
        except Exception as e:
            print(f"{Fore.RED}Ошибка сохранения JSON: {e}")

    def save_report_excel(self, output_path):
        if not pandas_available:
            print(f"{Fore.RED}Excel‑сохранение невозможно: pandas не найден.")
            return
        try:
            df = pd.DataFrame(self.results)
            df.to_excel(output_path, index=False)
            print(f"{Fore.GREEN}📁 Excel сохранён: {output_path}")
        except Exception as e:
            print(f"{Fore.RED}Ошибка сохранения Excel: {e}")
    ...
    def save_report_pdf(self, output_path):
        ...
        try:
            ...
            for i, res in enumerate(self.results, 1):
                ...
                if i > 100:  # ограничение на 100 записей в демо
                    pdf.cell(0, 5, '... (превышен лимит страниц)', ln=True)
                    break
...
if __name__ == "__main__":
    ...
    if auditor.results:
        ...
        # Предлагаем сохранить отчёт
        print(f"\n{Fore.CYAN}Доступные форматы для сохранения отчёта:")
        available = []
        ...
        choice = input(f"{Fore.CYAN}Выберите формат ({'/'.join(available)} или Enter для пропуска): {Style.RESET_ALL}").strip().lower()
        if choice:
            # Запрос места сохранения
            default_name = f"audit_results_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
            default_dir = Path(folder_input)
            dest = input(f"{Fore.CYAN}Введите путь для сохранения (по умолчанию {default_dir / default_name}): {Style.RESET_ALL}").strip()
            if not dest:
                if choice == 'csv':
                    dest = str(default_dir / (default_name + '.csv'))
                elif choice == 'json':
                    dest = str(default_dir / (default_name + '.json'))
                elif choice == 'excel' and pandas_available:
                    dest = str(default_dir / (default_name + '.xlsx'))
                elif choice == 'pdf' and fpdf2_available:
                    dest = str(default_dir / (default_name + '.pdf'))
                else:
                    print(f"{Fore.RED}Неверный формат.")
                    dest = None
            if dest:
                if choice == 'csv':
                    auditor.save_report_csv(dest)
                elif choice == 'json':
                    auditor.save_report_json(dest)
                elif choice == 'excel':
                    auditor.save_report_excel(dest)
                elif choice == 'pdf':
                    auditor.save_report_pdf(dest)
                else:
                    print(f"{Fore.RED}Неподдерживаемый формат.")
    else:
